...
void new_leaf(int leaf, char *buffer, int model)
{
    ...
    else if (leaf == 4 && (model == 85 || model == 88))
    {
        int i;
        int reconstruct[56] = {
	...
	else if (leaf == 4 && (model == 90 || model == 95))
    {
        int i;
        int reconstruct[56] = {
			0,0x6E,1,0x79,2,0x72,3,0x42,4,0x01,8,0x3E,12,0x6F,13,0xE8,
	...
	else if (leaf == 4 && model == 96)
    {
        int i;
        int reconstruct[78] = {
			0,0x6E,1,0x79,2,0x72,3,0x42,4,0x01,8,0x5E,12,0x63,13,0x1D,
            14,0x32,15,0x09,16,0xD8,18,0x24,20,0x14,21,0x31,22,0x14,23,0x1E,
            24,0x94,26,0x48,28,0xD8,52,0x80,53,0x0A,70,0x36,71,0x10,72,0xD2,
            73,0x0F,74,0x20,75,0x1C,76,0x3C,77,0x02,78,0x48,79,0x3C,80,0x51,
            81,0x4E,84,0x44,86,0x48,87,0x3C,88,0x51,89,0x4E,90,0x05
        };
        memset(buffer, 0, 512);
        for (i = 0; i < 78; i+=2)
            buffer[reconstruct[i]]=reconstruct[i+1];
    ...
    else if (leaf == 6 && (model == 82 || model == 85 || model == 88 || model == 90 || model == 91 || model == 95 || model == 96))
    {
        int i;
        int reconstruct[28] = {
	...
	else if(leaf == 0x41 && (model == 90 || model == 91 || model == 95 || model == 96))
	{
		int i;
		int reconstruct[178] = {
	...
	else if (leaf == 0x43 && ( model == 90 || model == 95 || model == 96 ))
    {
        int i;
        int reconstruct[122] = {
		0,0x55,1,0x73,2,0x74,3,0x72,4,0x53,5,0x4F,6,0x4E,7,0x59,
    ...
    else if (leaf == 0x45 && (model == 79 || model == 82 || model == 90 || model == 91 || model == 95 || model == 96))
    {
    	unsigned int b;
...
void analyze_leaves(void)
{
    ...
    while (1)
    {
        ...
        switch (check_mobo())
        {
            ...
            break;
            case 0x0303:
            printf(" The IdStorage identifies the motherboard as a TA-085/88\n\n");
            break;
            case 0x0404:
		...
		printf(" Please make sure your board is correct!\n IF it IS NOT DO NOT FIX LEAVES!!\n\n");
		pspDebugScreenSetTextColor(0x00FFFFFF);
		printf(" Press X to Continue, or O to quit.\n");
        b = wait_pressK(PSP_CTRL_CROSS|PSP_CTRL_CIRCLE);
        wait_releaseK(PSP_CTRL_CROSS|PSP_CTRL_CIRCLE);
        if (b & PSP_CTRL_CROSS && check_mobo() == 0x0101)
        {
            analyze_7981();
            return;
        }
        if (b & PSP_CTRL_CROSS && check_mobo() == 0x0202)
        {
            analyze_8286();
            return;
        }
        if (b & PSP_CTRL_CROSS && check_mobo() == 0x0303)
        {
            analyze_8588();
            return;
        }
        if (b & PSP_CTRL_CROSS && check_mobo() == 0x404)
		{
            analyze_91();
            return;
		}
		if (b & PSP_CTRL_CROSS && check_mobo() == 0x501)
		{
            analyze_90();
            return;
		}
		if (b & PSP_CTRL_CROSS && check_mobo() == 0x808)
		{
            analyze_95();
            return;
		}
...
int main(void)
{
    ...
    pspDebugScreenInit();
    pspDebugScreenSetBackColor(0x33AAAA00);
    pspDebugScreenSetTextColor(0x00ffffff);
    pspDebugScreenClear();
